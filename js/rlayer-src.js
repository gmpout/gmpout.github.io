...
L.Map.include({
	...
	_updateRaphaelViewport: function () {
		var	p = 0.02,
			...
			width = max.x - min.x,
			height = max.y - min.y,
			root = this._raphaelRoot.firstChild,
...
R.Pulse = R.Layer.extend({
	initialize: function(latlng, radius, attr, radius2, pulseAttr, options) {
		R.Layer.prototype.initialize.call(this, options);
        ...
        this._radius2 = (typeof radius2 == 'number' ? radius2 : 6);
	...
	projectLatLngs: function() {
		...
		this._pulse = this._paper.circle(p.x, p.y, this._radius2).attr(this._pulseAttr);
